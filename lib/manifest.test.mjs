...
test('archive items stay public but can leave the homepage flow', async () => {
  assert.ok((await getAllPosts()).some((p) => p.slug === '/archive/redis'));
  assert.ok(!(await getAllPosts({ includeArchive: false })).some((p) => p.slug === '/archive/redis'));
});
